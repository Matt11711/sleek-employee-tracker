    ...
    return new Promise(function (resolve, reject) {
      resolve(
        db
          .promise()
          .query(
            "SELECT employee_roles.title role, employee_roles.salary, departments.department_name department FROM employee_roles LEFT JOIN departments ON employee_roles.department_id = departments.id"
          )
          .then((rows, fields) => {
            return rows[0];
          })
